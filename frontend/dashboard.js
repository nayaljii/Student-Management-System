...
function getPercentage(student) {
    ...
    const obtained = student.marks?.obtained || 0;
    const outOf = student.marks?.outOf || 100;
...
function openCourseAttendanceSelector() {
    ...
    document.getElementById("courseSummaryList").innerHTML = html;
...
async function saveBulkAttendance() {
    ...
    try {
        ...
        const alreadyMarked = courseStudents.some(student =>
            student.dailyAttendance?.some(attendance => attendance.date === today)
        );
    ...
    for (const student of courseStudents) {
        ...
        const updatedHistory = (student.attendanceHistory || []).map(item => {
            if (item.month === month) {
                return {
                    ...item,
                    present: Math.max((item.present || 0) - 1, 0),
